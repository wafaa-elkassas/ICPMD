...
namespace ICPMD.Modules
{
    ...
    {        
        ...
        public static void resources_converter(string path, string project_name)
        {
            ...
            string path1 = target_path + "\\Abstract\\" + project_name + "\\Assets\\";
           ...
           // isExists = System.IO.Directory.Exists(source_path + "\\res");
            //if (isExists)
            //{
                //foreach (string f in Directory.GetFiles(source_path))
                //   {

                resource_directory(source_path, path1);
                ...
                //    {
                //        if (f1.EndsWith(".png") || f1.EndsWith(".jpg"))
                //        {
        ...
        public static void resource_directory(string dir, string path)
        {
            string xx ="";
            foreach (string f in Directory.GetFiles(dir))
            {
                if (f.EndsWith(".png") || f.EndsWith(".jpg") || f.EndsWith(".mp3"))
                {
                    try
                    {
                        bool isExists = System.IO.Directory.Exists(path + dir.Substring(dir.LastIndexOf("\\")));
                        if (!isExists)
                            xx = path + dir.Substring(dir.LastIndexOf("\\"));
                       
                        if (xx.Contains("drawable"))
                        {
                             File.Copy(f, path + f.Substring(f.LastIndexOf("\\")));
                        }
                        else{
                            System.IO.Directory.CreateDirectory(xx);
                            File.Copy(f, xx);
                        }
                    }
                    catch (Exception ex)
        ...
        public static void manifest_converter(string path, string project_name)
        {
            string target_path = path;
            string source_path = path + "\\Source\\" + project_name;
            string xmlPath = source_path + "\\AndroidManifest.xml";
            string xslPath = HttpContext.Current.Server.MapPath("~/XSLT/") + "\\Android_to_AM_Manifest.xslt";
            ...
            //XDocument xdoc = XDocument.Load(resultPath);
            ...
            node = xdoc.SelectSingleNode("/application/appinfo/publisherID");
            node.InnerText = "{" + Guid.NewGuid().ToString() + "}";

            //var element = xdoc.Elements("/application/appinfo/productID").Single();
            //element.Value = "{"+Guid.NewGuid().ToString()+"}";
            //var element1 = xdoc.Elements("/application/appinfo/publisherID").Single();
            //element.Value = "{" + Guid.NewGuid().ToString() + "}";
            xdoc.Save(target_path + "\\Abstract\\" + project_name + "\\Manifest\\manifest.xml");
        ...
        {
            string target_path = path;
            string source_path = path + "\\Source\\" + project_name+"\\res\\layout"; 
            foreach (string f in Directory.GetFiles(source_path))
                if (f.EndsWith(".xml"))
                {
                    //if (f.EndsWith("App.xaml"))
                    ...
                    string xslPath = HttpContext.Current.Server.MapPath("~/XSLT/") + "\\Android_to_AM_UI.xslt";
                    file_name = file_name.Substring(1, file_name.IndexOf(".") - 1);
                        
                    // change file name to match .java file
                    Regex rr = new Regex("[\\s]*setContentView[\\(]R.layout.([\\w]*)[\\)][\\s]*;", RegexOptions.None);
                    //-------------------------------------------------------- 
                    foreach (string d in Directory.GetDirectories(path + "\\Source\\" + project_name+"\\src"))
                {
                  if (Directory.Exists(d))
                        {
                            //string file_name = f.Substring(f.LastIndexOf("\\"));
                           new_file_name = ProcessDirectory1(d,file_name);
                        }
                    }
                    //--------------------------------------------------------
                     ...
                     //    if (mm.Success)
                     //    {
                     //       new_file_name = mm.Groups[1].ToString();
                     //    }
                     //}
                    ...
                    if (new_file_name !="")
                       resultPath = target_path + "\\Abstract\\" + project_name + "\\UI\\" + new_file_name + ".xml";
                    else
                       resultPath = target_path + "\\Abstract\\" + project_name + "\\UI\\" + file_name + ".xml";
                   ...
                   // if (new_file_name !="")
                        ...
                        //XslCompiledTransform loads XSLT file
                        XslCompiledTransform xsl = new XslCompiledTransform();
                        XsltSettings ss = new XsltSettings(true, false);
                        
                        xsl.Load(xslPath,ss, new XmlUrlResolver());
             ...
             source_path = path + "\\Source\\" + project_name+"\\res\\menu";
             if (Directory.Exists(source_path))
             {
                 foreach (string f in Directory.GetFiles(source_path))
                 {
                     if (f.EndsWith(".xml"))
                     {
                         ...
                         string file_name = f.Substring(f.LastIndexOf("\\"));
                         string xmlPath = source_path + "\\" + file_name;
                         string xslPath = HttpContext.Current.Server.MapPath("~/XSLT/") + "\\Android_to_AM_Menu.xslt";
                         string resultPath = target_path + "\\Abstract\\" + project_name + "\\Menu\\" + file_name + ".xml";

                         //XmlReader loads XML file
                         XmlReader reader = XmlReader.Create(xmlPath);

                         // create folder if not exist
                         bool isExists = System.IO.Directory.Exists(target_path + "\\Abstract\\" + project_name + "\\Menu");
                         if (!isExists)
       ...
       // }
         ...
         public static string ProcessDirectory1(string targetDirectory, string old_file_name)
        {
            foreach (string f in Directory.GetFiles(targetDirectory))
            {
                ...
                {
                    //string file_name = f.Substring(f.LastIndexOf("\\"));
                    string file_name = f.Substring(f.LastIndexOf("\\") + 1).Replace(".java", "");
                    ...
                    for (int j = 0; j < lines.Length; j++)
                    {
                        ...
                        if (mm.Success)
                        {
                            if (mm.Groups[1].ToString() == old_file_name)
                            { return file_name; }
            ...
            foreach (string d1 in Directory.GetDirectories(targetDirectory))
            {
                if (Directory.Exists(d1))
                {
                    //string file_name = f.Substring(f.LastIndexOf("\\"));
                   return ProcessDirectory1(d1, old_file_name);
                    //return string.Empty;
                }
            }
        ...
        {//string source_file_name,
            ...
            {
            foreach (string d in Directory.GetDirectories(source_path))
            {
                  if (Directory.Exists(d))
                        {
                            //string file_name = f.Substring(f.LastIndexOf("\\"));
                            ProcessDirectory(d,source_path, target_path, project_name);
                        }
                    }
                }
        ...
        public static void ProcessDirectory(string targetDirectory, string source_path, string target_path, string project_name)
        {
            foreach (string f in Directory.GetFiles(targetDirectory))
            {
                if (f.EndsWith(".java"))
                {
                    string file_name = f.Substring(f.LastIndexOf("\\"));
                    if (file_name.Equals("\\R.java")==false)
                    {
                        algorithm(targetDirectory, target_path, project_name, file_name);
                    }
                }
            }
             ...
             foreach (string d1 in Directory.GetDirectories(targetDirectory))
            {
                  if (Directory.Exists(d1))
                        {
                            //string file_name = f.Substring(f.LastIndexOf("\\"));
                            ProcessDirectory(d1,source_path, target_path, project_name);
                        }
                    }
                }
        ...
        public static void algorithm(string source_path, string target_path, string project_name, string file_name)
        {
            ...
            string[] temp_regex, temp_wp,lines_pattern_name;
            ...
            else
            {
                file_name = file_name.Substring(0, file_name.IndexOf(".") );
                        ...
                        //------------ search for UI xml file of this code file to rename it ---------------------------
                        //------------ code preparation phase -----------------------------------------------------------
                        for (int xx = 0; xx < raw_lines.Length; xx++)
                        {
                            if ((raw_lines[xx].Trim().EndsWith("{")) && (raw_lines[xx].Trim().Length > 1))
                            {
                                string[] sp = raw_lines[xx].Split('{');
                                loc = loc + (sp.Length - 1);

                                for (int yy = 0; yy < sp.Length; yy++)
                                {
                                    if (sp[yy].Trim().Equals("") == false)
                                    {
                            ...
                            if (raw_lines[xx].Trim().StartsWith("@Override") && (raw_lines[xx].Trim().Length == 9))
                            {
                                
                                string tem = raw_lines[xx].Trim() + " " + raw_lines[xx + 1].Trim();

                                if ((tem.Trim().EndsWith("{")))
                                {
                                    string[] sp = tem.Split('{');
                                    loc = loc + (sp.Length - 2);
                        ...
                        temp_wp = new string[lines.Length];
                        ...
                        while (reader10.Read() && reader10.HasRows)
                        {
                            ...
                            OleDbCommand command1 = new OleDbCommand("SELECT composite_pattern_statements.android_statement, composite_pattern_statements.item_id, composite_patterns.pattern, composite_patterns.Count, Component.component, composite_pattern_statements.pattern_id,composite_pattern_statements.wp_output,different_method_id FROM (composite_pattern_statements INNER JOIN composite_patterns ON composite_pattern_statements.pattern_id = composite_patterns.ID) LEFT JOIN Component ON composite_patterns.component_id = Component.ID  where composite_patterns.id=" + pattern_id + ";", connection);
                                 ...
                                 if (pattern_name.Equals(reader5.GetString(2)))
                                    ...
                                    //--------------- extract parameters and update temp_regex -------------------------
                                     ...
                                     if (!reader5.IsDBNull(6))
                                     {
                                         ...
                                         temp_wp[j] = reader5.GetString(6);
                                         if (temp_wp[j].Contains("@PARAM"))
                                         {
                                             foreach (DictionaryEntry pair in param)
                                             {
                                                 temp_wp[j] = temp_wp[j].Replace(pair.Key.ToString(), pair.Value.ToString());
                                             }
                                         }

                                         if (temp_wp[j].Contains("@SHARE"))
                                         {
                                             foreach (DictionaryEntry pair in share)
                                             {
                                                 temp_wp[j] = temp_wp[j].Replace(pair.Key.ToString(), pair.Value.ToString());
                                     ...
                                     OleDbCommand command22 = new OleDbCommand("SELECT android_expression, wp_code from expression_patterns;", connection);
                                     OleDbDataReader reader22 = command22.ExecuteReader();
                                     //Hashtable param22 = new Hashtable();
                                     //              int param22_index=1;
                                     string wp22;
                                     if (String.IsNullOrEmpty(temp_wp[j]) == false)
                                     {
                                         while (reader22.Read() && reader22.HasRows && (reader22.IsDBNull(0) == false))
                                         {
                                             ...
                                             //  {
                                             Match m22 = r22.Match(temp_wp[j]);
                                             if (m22.Success)
                                             {
                                                 wp22 = reader22.GetString(1);
                                                 //--------------- extract parameters and update temp_regex ----------------------
                                                 for (int xx = 1; xx < m22.Groups.Count; xx++)
                                                 {
                                                     //param22.Add("@PARAM" + param22_index, m22.Groups[xx].ToString());
                                                     wp22 = wp22.Replace("@PARAM" + xx, m22.Groups[xx].ToString());
                                                     //param22_index = param22_index + 1;
                                                 }
                                                 temp_wp[j] = temp_wp[j].Replace(m22.Groups[0].ToString(), wp22);
                                             }
                                             //-------------- update java code -----------------------
                                             //-------------------------------------------------------------------------------
                                             //  }
            ...
            //--------- end of composite pattern check -----------------------------------------------
                        ...
                        OleDbCommand command99 = new OleDbCommand("SELECT method_name, android_statement, wp_code_start, wp_code_end FROM method_to_class where  approved=true;", connection);// where id =" + strid, connection);
                        ...
                        OleDbCommand command111 = new OleDbCommand("SELECT  android_statement,id,wp_code FROM simple_patterns;", connection);
                        OleDbDataReader reader555 = command111.ExecuteReader();
                        while (reader555.Read() && reader555.HasRows && (reader555.IsDBNull(0)==false))
                        {
                            ...
                            for (int j = 0; j < lines.Length; j++)
                            {
                                if (lines_type[j].Equals("composite")==false)
                                {
                                    ...
                                    {                                              
                                        ...
                                        {
                                            temp_wp[j] = reader555.GetString(2);
                                            if (temp_wp[j].Contains("@PARAM"))
                                            {
                                                foreach (DictionaryEntry pair in param)
                                                {
            ...
            for (int x = 0; x < lines.Length; x++)
                        {
                            ...
                            if (lines[x].Trim().EndsWith("}") == true)
                            {
                                try
                                {
                                    ...
                                    if (page_class_end.Equals("pageclass") == true)
                                    {
                                        ...
                                        //    //    wp = "<wp>" + update_string(reader43.GetString(1)) + "</wp>";
                                        //    //    sr.WriteLine(wp);
                                        ...
                                        //            if (String.IsNullOrEmpty(temp_wp[s]) == false)
                                        //            {
                                        //                wp = "<wp>" +update_string( temp_wp[s]) + "</wp>";
                                        //            }
                                        //            sr.WriteLine(wp);
                                        //            sr.WriteLine("</pattern>");
                                        //        }
                                        ...
                                        //        sr.WriteLine("</comment>");
                                        //        sr.WriteLine("<pattern category=\"composite\"   name=\"new added method\"  type=\"suggested\"   source=\"ICPMD\">");
                                        //        wp = "<wp>" + update_string(reader43.GetString(3)) + "</wp>";
                                        //        sr.WriteLine(wp);
                                        //        sr.WriteLine("</pattern>");
                                        //    }
                                        //    sr.WriteLine("}");
                            ...
                            if (lines[x].Trim().StartsWith("/*") == true)
                            {
                                ...
                                while ((lines[x].Contains("*/") == false) && (comment_flag == false))
                                {
                                    sr.WriteLine(update_string(lines[x].ToString()));
                                    x = x + 1;
                                }
                            ...
                            else
                            {
                                ...
                                {
                                    int gg = 0;
                                    OleDbCommand command97 = new OleDbCommand("SELECT method_name, android_statement, wp_code_start, wp_code_end FROM method_to_class where  approved=true;", connection);// where id =" + strid, connection);
                                    ...
                                    while (gg == 0 && reader97.Read() && reader97.HasRows )
                                    {
                                        ...
                                        if (m2.Success)
                                        {
                                            ...
                                            wp = "<wp>" + update_string(pattern_desc) + "</wp>";
                                    ...
                                    OleDbCommand command9 = new OleDbCommand("SELECT android_statement, wp_code,tagname FROM simple_patterns where id=" + lines_pattern_id[x] + " and approved=true;", connection);// where id =" + strid, connection);
                                    ...
                                    while (gg==0 && reader9.Read() && reader9.HasRows)   
                                    {
                                        ...
                                        if (m.Success)
                                        {
                                          ...
                                          //  int index = 1;
                                            ...
                                            wp = "<wp>" + update_string(temp_wp[x]) + "</wp>";
                                ...
                                {
                                    OleDbCommand command4 = new OleDbCommand("SELECT composite_pattern_statements.android_statement, composite_pattern_statements.wp_output, composite_patterns.pattern,composite_pattern_statements.id FROM composite_pattern_statements INNER JOIN composite_patterns ON composite_pattern_statements.pattern_id = composite_patterns.ID  where composite_pattern_statements.pattern_id=" + lines_pattern_id[x] + ";", connection);
                                    bool flag1 = false;
                                        ...
                                        if (m.Success)
                                        {
                                          ...
                                          //--------- check if statement should appear in another method ---------------------
                                            ...
                                            else
                                            {
                                                ...
                                                //sr.WriteLine("<wp>"+ reader9.GetString(1)+"</wp>");

                                                if (String.IsNullOrEmpty(temp_wp[x]) == false)//(!reader4.IsDBNull(1))
                                                {
                                                    // wp = "<wp>" + reader4.GetString(1) + "</wp>";
                                                    wp = "<wp>" + update_string(temp_wp[x]) + "</wp>";
                                                }
                                ...
                                else
                                {
                                    ...
                                    OleDbCommand command = new OleDbCommand("SELECT android_regex,action_id,item,id,component_name,component_method FROM repository where approved=true", connection);// where id =" + strid, connection);
                                    ...
                                    while (reader.Read() && reader.HasRows )
                                    {
                                        ...
                                        while (m.Success)
                                        {
                                            ...
                                            if (reader.GetInt32(1) == 2)
                                            {
                                                ...
                                                if (tagname == "pageclass")
                                                {
                                                    sr.WriteLine("<param name=\"projectname\"   value=\"" + project_name + "\" />");
